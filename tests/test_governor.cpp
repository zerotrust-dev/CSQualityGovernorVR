...
TEST_CASE("a step's cost is not measured through the transition spike", "[governor]")
{
	// D-19, and the reason it exists. The post-change P95 used to be taken as
	// soon as the window was full - 30 samples, 0.42 s at 72 fps - while settle
	// takes ~1.0 s (E-2). A settling transition only ever ADDS GPU time, so the
	// measurement was inflated every time rather than sometimes, and smoothing
	// more observations converged on the wrong number (E-31: 1.146 believed
	// against 1.092 observed, over seven observations).
	//
	// Here the true rung costs 20%, and the transition briefly costs 60%. A
	// controller that measures through the spike learns something well above
	// 1.20; one that waits for quiet learns 1.20.
	auto config = TestConfig();
	config.stepRatioAlpha = 1.0;  // adopt each observation outright, so the test
	config.maxClimbRungs = 1;     // reads the measurement and not an average

	Harness h{ config, Preset::Balanced };

	// A ladder with a settling transition on top of it. Harness::Run feeds a
	// constant, which cannot express a transient at all - the very thing this
	// decision is about - so the loop is written out here.
	const auto quietCost = [](Preset a_preset) {
		switch (a_preset) {
		case Preset::Balanced:
			return 9.0;
		case Preset::Quality:
			return 9.0 * 1.20;  // the truth this test is about
		default:
			return 13.0;  // expensive enough that the ladder stops climbing
		}
	};

	static std::uint64_t index = 4'000'000;
	double lastChange = -100.0;
	for (int i = 0; i < 700; ++i) {
		h.now += 0.0139;
		const double base = quietCost(h.preset);
		// The transition costs 15% extra for a second: DLSS history reset and
		// resource work, the same thing E-2 measured as ~1.0 s of settle.
		const bool settling = h.now - lastChange < 1.0;
		GovernorSample sample;
		sample.nowSeconds = h.now;
		sample.frameTimeMs = kBudget;
		sample.gpuTimeUs = static_cast<std::uint64_t>((settling ? base * 1.15 : base) * 1000.0);
		sample.gpuFrameIndex = ++index;
	...
	// Measured through the transient the ratio would be about 1.38; the rung
	// actually costs 1.20.
	CHECK(h.core.StepRatio(Preset::Balanced) == Approx(1.20).margin(0.06));
	CHECK(h.core.StepRatio(Preset::Balanced) < 1.30);
}

TEST_CASE("a transition that never settles teaches nothing", "[governor]")
{
	// The cost of D-19, made explicit rather than discovered later: if GPU time
	// never holds still, no measurement is taken at all. That is the intended
	// trade - a step that is never measured keeps its seed, and D-18's seeds are
	// deliberately conservative, so the failure mode is staying put rather than
	// adopting a number produced by a scene that was moving throughout.
	auto config = TestConfig();
	config.stepRatioAlpha = 1.0;
	config.maxClimbRungs = 1;

	Harness h{ config, Preset::Balanced };
	const auto seed = h.core.StepRatio(Preset::Balanced);

	// Quiet at Balanced so the climb happens and the "before" half is real;
	// then noise at Quality far wider than the settle tolerance, for long
	// enough that the old code would certainly have measured and learned. The
	// noise stays under budget so this tests the calibration and not a descent.
	static std::uint64_t index = 5'000'000;
	for (int i = 0; i < 700; ++i) {
		h.now += 0.0139;
		const double gpu =
			h.preset == Preset::Balanced ? 9.0 : (i % 2 == 0 ? 10.0 : 12.6);
		GovernorSample sample;
		sample.nowSeconds = h.now;
		sample.frameTimeMs = kBudget;
		sample.gpuTimeUs = static_cast<std::uint64_t>(gpu * 1000.0);
		sample.gpuFrameIndex = ++index;
		if (auto decision = h.core.Push(sample, h.preset)) {
			if (decision->action != GovernorAction::Hold) {
				h.changes.push_back(*decision);
	...
	REQUIRE_FALSE(h.changes.empty());
	CHECK(h.changes.front().target == Preset::Quality);
	CHECK(h.core.StepRatio(Preset::Balanced) == Approx(seed));
}
