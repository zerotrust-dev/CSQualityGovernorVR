...
D-18 replaced D-17's single fitted `k` with six measured per-step ratios, citing
E-27: the implied `k` ran from 0.21 to 2.27 across the ladder, so "no single
value fits both ends and the linear cost model misfits by construction."

**That conclusion does not follow from that evidence.** E-27 estimated `k`
*pairwise* — from two adjacent presets at a time. Adjacent presets differ in
pixel fraction by as little as 0.096, so a two-point slope divides measurement
noise by a small number and multiplies it. E-51 reproduces E-27's spread exactly
in character from a single session (1.53 to 18.74 ms per unit pixel fraction, a
12.3× range) using data whose *global* fit over all seven presets is stable to
1.1× and describes every preset to within 0.39 ms. The linear model was never
the thing that failed. The estimator was.

This matters beyond bookkeeping: D-18 gave up generalisation — "each step must be
seen at least once" — to escape a defect that a better-conditioned fit of the
same model would have avoided.

**The evidence against per-step learning (E-51).** Two sweeps, one session, ten
minutes apart:

| rung | measured s1 | measured s2 | model s1 | model s2 |
|---|---|---|---|---|
| UltraPerformance → Performance | 1.083× | 1.247× | 1.213 | 1.157 |
| Performance → Balanced | 1.153× | 1.087× | 1.121 | 1.094 |
| Balanced → Quality | 1.113× | 1.037× | 1.111 | 1.088 |
| Quality → UltraQuality | 1.204× | 1.112× | 1.149 | 1.121 |
| UltraQuality → Hoshipa | 1.013× | 1.140× | 1.115 | 1.096 |

Mean spread between the two sweeps: **measured 0.105, model 0.030.** Learning a
rung from its own transitions is 3.5× noisier than computing it from a fit of the
whole ladder, because a single transition measures the scene as much as the step.

**The proposal.**

```
t_target = t_now  x  (A + B·f_target) / (A + B·f_now)          f = scale²
```

`A` and `B` are fitted by least squares over all seven presets from the
calibration sweep's dwell frames — which the sweep already measures and currently
writes to a summary file and discards. `t_now` is the live P95 the controller
already has.

**Fit on P95, not on the mean.** E-28's lesson: a prediction and the thing it
predicts must be in the same currency, and the controller decides on P95.

**Deletes:** `_stepRatio`, `_stepMeasured`, `_stepObservations`,
`LearnStepRatio`, `stepRatioAlpha`, `stepRatioMin`, `stepRatioMax`, the seven
seeds, and the `_p95BeforeChange` / `_awaitingCalibration` two-point machinery.
The multi-rung walk stops being a product of noisy ratios and becomes a lookup:
the highest `f` whose predicted cost fits. `maxClimbRungs` survives as a safety
clamp rather than as a mechanism.

**What re-anchoring assumes, and what it does not.** Dividing by
`(A + B·f_now)` absorbs the overall level of the scene, so the fit does not have
to predict absolute cost — only the *shape*. It does assume a heavier scene
raises fixed and scaled cost roughly together. E-51 shows that is only
approximately true: the fixed/scaled ratio moved 0.54 → 0.77 between the two
sweeps. The model-vs-model spread of 0.030 above is exactly the residual error
that assumption leaves behind, and it is still 3.5× better than the alternative.

**Answers a question the current design cannot.** The seeds are baked at build
time from one machine, one resolution and one mod list. Change the headset's
render resolution — Pimax's 72 Hz Upscale mode, for instance — and `B` moves
while the seeds do not. A per-session fit re-measures it and the governor carries
on; the shipped table cannot, and would be wrong for the whole session.

**Expected effect, stated in advance.** Landing-prediction error should fall by
roughly the ratio above. Climb-then-reverse pairs should become rarer for the
right reason — the prediction being right, rather than D-20 refusing on memory.
Multi-rung landings should improve most, since they currently multiply several
noisy numbers.

**Refutation condition.** Replay both committed captures and compare predicted
landing against realised P95 after settle. If the fitted model's error is not
materially below D-18's per-step error, the extra machinery has bought nothing
and the proposal should be withdrawn rather than tuned. Likewise if time-weighted
pixel fraction falls without a matching fall in changes.

**Cost if wrong.** A model that misfits degrades every rung at once, where the
table degrades one. Mitigation: keep the session's own fit residual, and when it
exceeds a threshold widen `landingMarginMs` rather than trusting the curve — the
fit publishes its own error, which the per-step table never could. If the sweep
is skipped, there is no fit; fall back to shipped `A`/`B` and say so in the log
rather than pretending to a measurement.

**What it does not fix.** Nothing here addresses E-49: frames that miss while GPU
time reads under budget. That cost is invisible to every measurement in this
process, and a better cost model for the ladder does not make it visible. Two
separate problems; this is only the first.

### D-20 — A climb that was just reversed is not re-tried at the same headroom
...
| E-62 | **VR Render Scale Mode is worth 1.6-2.5 ms, and the earlier "no detectable benefit" was measuring nothing.** Verified pair, 2026-08-15, `renderScaleMode` read from `SettingsUser.json` for both arms per Rule 12. Sweep dwells, deduplicated nearest-rank P95: UltraPerformance **8.65 -> 11.02 ms (+2.37)**, Performance **10.81 -> 13.28 (+2.47)**, Balanced 12.65 -> 14.28 (+1.63), Quality 13.71 -> 15.40 (+1.69) when the feature is turned off. Gameplay steady state agrees at +0.8 to +1.7. And the pattern is physically coherent - the saving **shrinks as quality rises**, 22% at UltraPerformance where targets are 11% of full size against 11% at Quality where they are 44% - which is what allocating smaller targets must do and is not a shape noise produces. **This overturns the null result recorded on 2026-08-13** (+0.12 ms at Performance, -0.01 at UltraPerformance, "under 0.3 ms, abandon the prototype"). That A/B almost certainly had **both arms render-scale-on**: its NativeAA control showed a 0.88 ms spread where the true value is zero by construction, whereas a genuine configuration difference would have left NativeAA at ~0 and the rest at 1.6-2.5. The setting was never read - the same failure Rule 12 was written for, committed one session before the rule. **Consequence: the two configurations are each incomplete.** Render Scale Mode ON gives pixel fraction **0.320** with 1.35% of frames beyond two display periods, 85% of them a relatch within 0.7 s of a change. OFF gives **0.216** with 0.03% - baseline - severe frames. The controller is the same in both; the headroom that funded the quality is the feature. Hot-Envelope would give both, and its prize is now measured rather than argued. | Sessions `20260815_104325` (on) and `20260815_112602` (off), settings verified |
| E-61 (WITHDRAWN — the premise was false) | **The premise "Render Scale Mode is off" was never verified, and it was on.** `SettingsUser.json` in the active CSX preset mod reads `"renderScaleMode": 1`, last written 2026-08-13 20:18 - the RSM-ON arm of the A/B, which was never turned back off. So every adaptive session (14:08, 14:21, and 2026-08-14 10:45) ran WITH Render Scale Mode enabled, and the 57.2 ms down-transition spike attributed below to the DLSS viewport cache is simply the render-target relatch, whose signature we had already measured at 57-59 ms. The reasoning was "RSM is off, so no relatch is possible, so it must be the cache" - and the first clause was an assumption stated as fact. The cache may still become the next hitch once the relatch is genuinely gone; nothing here shows that it has. **Original text follows.** **With the relatch gone, the next hitch is the DLSS viewport cache — and it is directional.** Measured during a sweep with VR Render Scale Mode **off**, so no render-target relatch is possible: transitions **down** the ladder spike to a mean worst-frame of **26.8 ms and a max of 57.2 ms**, while transitions **up** reach 22.0 ms and 35.3 ms. A 57 ms frame is relatch-sized, and there is no relatch. This is the hitch Codex predicted when reviewing the render-scale architecture: CSX keeps only **two** VR DLSS viewport slots per role, keyed on quality mode plus preset, so cycling seven presets evicts a slot on nearly every change and each eviction waits for D3D11 retirement. It fits the player report exactly - a big freeze during calibration, where all seven presets cycle, and only the ~42 ms micro freeze during play, where two or three presets are in use and the cache hits. **Actionable without touching CS:** the sweep does not need every rung. Restricting `Order` to the operating range (`4,3,2,1`) measures every rung the controller can use, halves the sweep, removes the two most expensive presets from it entirely, and leaves the cache four presets to juggle instead of seven. | Sweep transitions, session 2026-08-14 14:21 |
| E-60 | **D-25/D-26 work mechanically on the first try; the operating point did not, and the brake was not the threshold.** First live adaptive sessions, 2026-08-14. **D-26 measured every rung** from the sweep, 936-1119 deduplicated dwell samples each: UltraPerformance→Performance **1.68 ms (12.1% of budget)**, Performance→Balanced 1.63 ms (11.8%), Balanced→Quality 0.47 ms (3.4%), Quality→UltraQuality 1.84 ms (13.3%), UltraQuality→Hoshipa 0.15 ms (1.1%), Hoshipa→NativeAA 3.92 ms (28.3%). So the self-confirming seed problem is solved: prices now come from this machine, this session. **But `climbMarginFrac = 5%` was wrong, and wrongly derived** — taken from simple mode's `20% ≈ 15.1% price + 5% margin`, which is **mean**-based arithmetic applied to a **P95** controller. P95 headroom runs 4-8 points below mean headroom, so the controller asked 17.1% where ~15.7% was available and declined climbs simple mode took and held. Logged repeatedly as `hold: 16.6% spare, need 17.1%` - missing by half a point. Result: pixel fraction **0.167** against a 0.256 baseline, a 35% quality loss, with late frames down 60% to 2.1%. At margin 3: **0.197**, late 4.4%, but **7.1 changes/min** against 3.7. **The dominant brake is not the threshold at all** - hold reasons across the session: **38 × `clean for Ns`** (the 8 s post-descend gate), 34 × cooldown, and only **8 ×** the threshold itself. And the cause beneath that: **Performance naturally runs at 4.2-4.4% late while `descendMissRate` was 5%**, so ordinary noise knocked it down a rung and the clean gate then held it there. The player independently reported "too slow" and "15-28% overhead for shorter periods", which is the same finding from the other side of the headset. | Sessions 2026-08-14 14:08 (margin 5) and 14:21 (margin 3) |
| E-59 | **The transition artefact is a hidden-area-mask rectangle at the nose, about 3 frames long, and it sets the floor on the hold.** With the relatch gone (E-58), `TransitionHoldFrames` was binary-searched against what the player can actually see, because a replayed frame arrives on time and is invisible in `frame_ms` — the eye is the only instrument for it. **8 clean, 3 clean, 2 shows it, 1 shows it.** The artefact is not a full-screen white flash: it is a **rectangle in the nose region of both eyes, white in the right eye and empty in the left** — and the nose is exactly what the hidden area mask covers. **Corrects a standing project note** which had this as an upstream bug fixed 2026-07-25 against an installed build of 2026-07-09: RC3 ships **CSX 3.18-VR dated 2026-08-09**, three weeks after that fix, and the artefact is still present. Either the fix missed this path, it regressed, or this is a similar-looking different artefact — a build date is not evidence it is gone. **SETTLED: the artefact is Community Shaders', not ours.** `TransitionHoldFrames = 0` was run, removing our capture and replay entirely, and the rectangle came back **wider, extending to the edges**. That refutes the hypothesis that our replay was exposing unwritten pixels outside the dynamic-resolution sub-rect - removing the replay made it worse, not better. The shape of the sequence is now clear: **the artefact starts wide and contracts to the nose over about three frames** (hold 0 shows it widest, 1 and 2 show progressively smaller nose rectangles, 3 shows nothing), which is what something being progressively re-established over three frames looks like. Our hold is pure concealment and 3 is exactly its length. This is a clean, small upstream bug report. | Sessions 2026-08-13 at holds 8/3/2/1, player observation |
| E-58 | **The transition freeze is removable by a Community Shaders setting, with no code change on either side — and the cost of doing so is unresolved.** The 85 ms relatch is not DLSS switching; it is a global engine rebuild triggered because CSX's VR Render Scale Mode makes Skyrim allocate its physical render targets at the selected quality's input resolution. Verified in `Features/Upscaling/PerfMode.cpp`: `restartRequired = boot.active && (... || boot.qualityMode != qualityMode)`, and `EnsureBootLatch` returns before setting `boot.valid`/`boot.active` when `IsEligible` is false, which it is when `renderScaleMode == 0`. So **booting with "VR Render Scale Mode" disabled means a quality change can never set `restartRequired`.** Measured, worst frame within 0.5 s of a preset change: **59.1-59.9 ms with it on, 33.7 ms with it off** across three sessions. Late frames *at the same preset* are unchanged (Performance 4.2% on, 4.4% off), preset distribution is comparable, and the player reports quality feeling unchanged. **The remaining "tiny freeze" is ours, not CS's:** `TransitionHoldFrames = 8` replays a captured frame for 111 ms, which holds the image while frame *times* stay normal - invisible in frame_ms, visible to the eye. It was sized to conceal a relatch that no longer happens. **The GPU cost is genuinely unresolved and two measurements disagree:** sweep dwells say +0.80 ms flat across four presets; gameplay frames say +0.04 ms at Performance and **-0.39 ms** at UltraPerformance, and the player independently noticed *more* headroom. Both are confounded by route and scene - the same sessions differ by 2.3 ms the other way at Quality. Only a same-location A/B settles it, and the decision does not appear sensitive to it. **The rung costs are unchanged either way (1.65 ms vs 1.62 ms), so D-26 is unaffected.** | CSX source `PerfMode.cpp`; sessions `20260813_172109` (off) against `20260813_133710` and `20260812_213623` (on) |
...
| E-53 | **OpenComposite does not populate the compositor's delivery counters, and no patch can make it — which removes the primary telemetry channel the safe-ladder architecture is built on.** Capability test (D-25, `PROPOSED_SAFE_LADDER_GOVERNOR.md` §17.2), one session: `GetFrameTiming` was called **18663 times, returned true on 100% of them, and the frame index advanced on 100%** — so the entry point is real and live, not a stub. `m_flClientFrameIntervalMs` and `m_flCompositorRenderGpuMs` vary, so the runtime does fill what it can compute itself. But **`m_nNumFramePresents`, `m_nNumDroppedFrames`, `m_nNumMisPresented` and `m_nReprojectionFlags` never changed once** across 18662 fresh frames — in a session containing **5478 late frames (31%), 121 of them longer than two display periods, with individual frames at 35 ms.** A 35 ms frame at 72 Hz means the previous image was scanned out at least twice, which is the documented meaning of `m_nNumDroppedFrames`; it reported zero. **The cause is structural, not a defect:** OpenComposite translates OpenVR onto OpenXR, and core OpenXR exposes no portable result for which image was scanned out, so there is no source for those fields to carry. No fix downstream can invent them. Consequences: the provider hierarchy in §5.2 has **no primary provider on this stack**, the delivery gating in §8 has no data, and D-25's use of the application frame interval is the *only* delivery signal available rather than a fallback compromise. | Session 2026-08-12 20:55, `rc3-2026-08-12-2055` |
...
| E-47 | **The transition hold works — and the deferral it needed made the governor believe every change had failed.** First clean session: **182 submits replayed, 0 withheld**, and the player reports no flash, no black and no artefact at any point — *"only BAM! the new quality was shot in"*, across per-eye and combined layouts alike. The visible freeze measures **96–130 ms**, covering an 85 ms relatch. But the timeline records **four `readback-mismatch` and zero `applied`**: D-23b defers the apply by a frame or two while a known-good frame is captured, and the verification reads the preset back immediately, before it has been applied. The changes did land — that is what the player saw — but the governor was told they were refused, and a governor whose changes appear to be refused stops making them, which is why it went quiet late in the session. Fixed by treating a deferred apply as pending rather than failed, verified when it lands. **A mitigation that changes the timing of an action has to tell whoever verifies that action.** | Session 2026-08-11 20:31 |
| E-46 | **The capture was never consumed, so every change after the first replayed the one before.** Deferring the change until a capture existed (E-45) turned the artefact from a gridded panel into a **black** window of the same length — and the player's description gave it away: *"the first 10 seconds everything works as it should"*. `CaptureReady()` stayed true once satisfied, so the second change found it already ready, skipped arming a fresh capture, and replayed the **previous** change's frame — stale by one preset and, across the per-eye/combined switch, by one layout. The first transition of a session captured fresh and looked right; every one after it did not. Fixed with a consumed flag: a hold claims the capture, and the next change must arm a new one. **Three of the last four defects here have been lifetime errors on the same object** — captured-per-eye (E-43), captured-with-its-bounds (E-44), captured-and-consumed (E-46) — which is what a design that keeps state across an event looks like when the state's ownership is never written down. | Player report, session 2026-08-11 20:4x |
